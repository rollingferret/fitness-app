...
function MileTimeGraph({ runs }) {
  ...
  useEffect(() => {
    ...
    const xScale = d3.scaleLinear().domain([0, runs.length - 1]).range([0, width]);
    ...
    // Transition to animate the line drawing
    path.transition().duration(2000).ease(d3.easeLinear).attr('stroke-dashoffset', 0);
